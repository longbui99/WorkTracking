...
class WorkTask(http.Controller):
    ...
    @handling_req_res
    @http.route(['/management/issue/get/<int:task_id>'], type="http", cors="*", methods=['GET'], csrf=False, auth='jwt')
    ...
    def get_related_active(self, **kwargs):
        active_task_ids = request.env['work.task'].get_all_active(json.loads(request.params.get("payload", '{}')))
    ...
    def add_task_work_log(self, **kwargs):
        task_id = self.check_work_log_prerequisite()
        task_id.generate_progress_work_log(request.params.get('payload', {}))
        data = self._get_task(task_id)
        return http.Response(json.dumps(data), content_type='application/json', status=200)
    ...
    def pause_task_work_log(self, **kwargs):
        task_id = self.check_work_log_prerequisite()
        task_id.action_pause_work_log(request.params.get('payload', {}))
        return http.Response("", content_type='application/json', status=200)
    ...
    def done_task_work_log(self, **kwargs):
        ...
        task_id = self.check_work_log_prerequisite()
        task_id.action_done_work_log(request.params.get('payload', {}))
        return http.Response("", content_type='application/json', status=200)
    ...
    def manual_task_work_log(self, **kwargs):
        request.params = json.loads(request.httprequest.data)
        task_id = self.check_work_log_prerequisite()
        task_id.action_manual_work_log(request.params.get('payload', {}))
        return http.Response("", content_type='application/json', status=200)
    ...
    def cancel_task_work_log(self, **kwargs):
        task_id = self.check_work_log_prerequisite()
        task_id.action_cancel_progress(request.params.get('payload', {}))
        return http.Response("", content_type='application/json', status=200)
    ...
    def get_acceptance_criteria(self, **kwargs):
        task_id = self.check_work_log_prerequisite()
        data = task_id.get_acceptance_criteria(request.params.get('payload', {}))
        return http.Response(json.dumps(data), content_type='application/json', status=200)
    ...
    def delete_acceptance_criteria(self, **kwargs):
        request.env['work.ac'].browse(request.params.get('acID', 0), ).unlink()
        data = {'status': 'ok'}
        return http.Response(json.dumps(data), content_type='application/json', status=200)