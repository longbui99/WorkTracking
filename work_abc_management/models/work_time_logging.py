...
class WorkTimeLog(models.Model):
    ...
    task_id = fields.Many2one('work.task', string='Task', ondelete="cascade")
    duration = fields.Integer(string='Duration', required=True)
    cluster_id = fields.Many2one('work.work.log.cluster')
    state = fields.Selection([('progress', 'In Progress'), ('done', 'Done')], string='Status', default='progress')
    source = fields.Char(string='Source')
    user_id = fields.Many2one('res.users', string='User')
    epic_id = fields.Many2one("work.task", string="Epic", related="task_id.epic_id", store=True)
    ...
    def unlink(self):
        ...
        work_log_ids = self.mapped('task_id').mapped('work_log_ids').filtered(lambda r: r.cluster_id in cluster_ids)
