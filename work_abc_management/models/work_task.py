...
class WorkProject(models.Model):
    ...
    _order = 'task_sequence desc, sequence asc, create_date desc'

    pin = fields.Boolean(string='Pin', copy=False)
    sequence = fields.Integer(string='Sequence', copy=False)
    task_name = fields.Char(string='Name', required=True)
    task_key = fields.Char(string='Task Key', copy=False)
    task_url = fields.Char(string='Task Task', copy=False)
    epic_id = fields.Many2one("work.task", string="Epic")
    epic_ok = fields.Boolean(string="Epic?", related="task_type_id.epic_ok", store=False)
    time_log_ids = fields.One2many('work.time.log', 'task_id', string='Log Times', copy=False)
    story_point = fields.Float(string='Estimate', copy=False)
    story_point_unit = fields.Selection([('general', 'Fibonanci'), ('hrs', 'Hour(s)')], string="Estimate Unit", default="general", copy=False)
    ...
    status_id = fields.Many2one('work.status', string='Status', copy=False)
    duration = fields.Integer('Duration', compute='_compute_duration', store=True, copy=False)
    progress_cluster_id = fields.Many2one('work.work.log.cluster', string='Progress Cluster', copy=False)
    work_log_ids = fields.One2many('work.work.log', 'task_id', string='Work Log Statuses')
    active_duration = fields.Integer("Active Duration", compute='_compute_active_duration', copy=False)
    my_total_duration = fields.Integer("My Total Duration", compute="_compute_my_total_duration", copy=False)
    last_start = fields.Datetime("Last Start", compute="_compute_last_start", copy=False)
    task_sequence = fields.Integer('Task Sequence', compute='_compute_task_sequence', store=True, copy=False)
    start_date = fields.Datetime("Start Date", copy=False)
    parent_task_id = fields.Many2one("work.task", string="Parent")
    log_to_parent = fields.Boolean("Log to Parent?")
    children_task_ids = fields.One2many("work.task", "parent_task_id", store=False, copy=False)
    ...
    personal = fields.Boolean(string="Is Personal", default=False, copy=False)
    company_id = fields.Many2one('res.company', string='Company', related='project_id.company_id', store=True, copy=False)
    description = fields.Text(string="Description", copy=False)
    priority_id = fields.Many2one("work.priority", string="Priority", copy=False)
    ...
    def _name_search(self, name, domain=None, operator='ilike', limit=None, order=None):
        if name and operator in ('=', 'ilike', '=ilike', 'like', '=like'):
            domain = domain or []
            domain = ['|', ('task_name', operator, name), ('task_key', operator, name)]
            return self._search(expression.AND([domain, domain]), limit=limit)
        return super(WorkProject, self)._name_search(name=name, domain=domain, operator=operator, limit=limit,
                                                     order=order)
    ...
    @api.depends('task_key')
    def _compute_task_sequence(self):
        for record in self:
            if record.task_key:
                record.task_sequence = int(record.task_key.split('-')[1])
    ...
    def generate_progress_work_log(self, values={}):
        ...
        for record in self:
            ...
            if not time_log_ids:
                cluster = self.env['work.work.log.cluster'].create({
                    'name': self.task_key + "-" + str(len(record.time_log_ids) + 1)
                })
    ...
    def _get_suitable_log(self):
        record = self
        while record.parent_task_id and record.log_to_parent:
            record = record.parent_task_id
        return record

    def action_done_work_log(self, values={}):
        self.action_pause_work_log(values)
        source = values.get('source', 'Internal')
        change_records = self.env['work.task']
        for task in self:
            record = task._get_suitable_log()
            suitable_time_log_pivot_id = record.time_log_ids.filtered(
                lambda r: r.user_id == self.env.user
                          and r.state == 'progress'
                          and r.source == source
    ...
    def action_manual_work_log(self, values={}):
        source = values.get('source', 'Internal')
        log_ids = self.env['work.time.log']
        change_records = self.env['work.task']
        ...
        for task in self.with_context(bypass_rounding=True):
            record = task._get_suitable_log()
            log_ids |= record.env['work.time.log'].create({
                'description': values.get('description', ''),
    ...
    def get_all_active(self, values={}):
        ...
        active_time_ids = self.env['work.time.log'].search([('user_id', '=', self.env.user.id),
                                                            ('source', '=', source),
                                                            ('state', '=', 'progress')])
        active_task_ids = (active_time_ids.mapped('task_id') - except_ids).ids
        self.search([('id', 'in', active_task_ids)], order=employee.order_style,
                    limit=employee.maximum_relative_result)
        if values.get('limit', False):
            active_task_ids = active_task_ids[:values['limit']]
    ...
    def get_search_task_domain(self, res, employee):
        ...
        if 'personal' in res:
            ...
            if user_dt:
                ...
                return [('id', '=', 0)]
        if 'favorite' in res:
            favorite_ids = self.env['work.task'].search([('id', 'in', self.env.user.employee_id.favorite_task_ids.ids)]).ids
            if favorite_ids:
                return [('id', 'in', favorite_ids)]
        if 'task' in res:
            domain = expression.AND([domain, [('task_key', 'ilike', res['task'])]])
        if 'project' in res:
            project_domain = []
            ...
            if 'sprint' in res:
                project_id = self.env["work.project"].search([('project_key', '=', res['project'])], limit=1)
                if project_id:
        ...
        if 'text' in res:
            domain = expression.AND([domain, [('task_name', 'ilike', res['text'])]])
        if 'name' in res:
    ...
    def search_task_by_criteria(self, payload):
        urls = find_url(payload)
        if len(urls):
            _, payload = self.env['work.base.integration'].get_host_and_task_by_query(payload)
        employee = self._get_result_management()
        res = get_search_request(payload)
        domain = self.get_search_task_domain(res, employee)
        result = self.env["work.task"]
    ...
    def get_daily_tasks(self, date):
            ...
            task = self.search([('project_id.personal_id', '=', self.env.user.id), ('applicable_date', '=', user_date)], limit=1) 
            if not task:
                project = self.env['work.project'].sudo().gather_personal_project()
                task = self.sudo().create({
                    'task_name': "Task: %s" % user_date.strftime("%B %d, %Y"),
                    'task_key': project.project_key + "-%s"% user_date.strftime("%m%d%y"),
                    'task_sequence': user_date.year + user_date.month + user_date.day,
                    'project_id': project.id,
                    'assignee_id': self.env.user.id,
                    'task_type_id': self.env['work.type'].search([('default_personal', '=', True)], limit=1).id,
                    'personal': True,
    ...
    def _personal_todo_move(self):
        yesterday_tasks = self.search([('personal', '=', True), ('applicable_date', '=', fields.Date.context_today(self) - relativedelta(days=1))])
        today_tasks = self.search([('personal', '=', True), ('applicable_date', '=', fields.Date.context_today(self))])
        empty_daily_task_user_ids = yesterday_tasks.assignee_id - today_tasks.assignee_id
        for user in empty_daily_task_user_ids:
            _self = self.with_user(user)
            _self.get_daily_tasks(fields.Date.context_today(_self))
        today_tasks = self.search([('personal', '=', True), ('applicable_date', '=', fields.Date.context_today(self))])
        today_task_by_user = {task.assignee_id.id: task for task in today_tasks}
        if yesterday_tasks:
            empty_yesterday_tasks = yesterday_tasks.filtered(lambda r: not r.ac_ids)
            yesterday_tasks -= empty_yesterday_tasks
            checklists = self.env['work.ac'].search([('task_id','in', yesterday_tasks.ids), ('checked', '=', False), ('is_header', '=', False)])
            checklists_by_task = defaultdict(lambda: self.env['work.ac'])
            for checklist in checklists:
                checklists_by_task[checklist.task_id] |= checklist
            for task, checklists in checklists_by_task.items():
                if checklists and task:
                    today_task = today_task_by_user.get(task.assignee_id.id)
                    to_update_acs = self.env['work.ac']
                    step_acs = self.env['work.ac']
                    if today_task:
                        for checklist in task.ac_ids[::-1]:
                    ...
                    to_update_acs.task_id = today_task.id
