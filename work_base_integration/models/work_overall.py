...
from odoo.addons.work_abc_management.models.work_time_logging import WorkTimeLog as WorkTimeLogBase

_logger = logging.getLogger(__name__)


class WorkStatus(models.Model):
    _inherit = "work.status"

    work_key = fields.Char(string='Task Key')
...
class WorkTimeLog(models.Model):
    ...
    def batch_export(self, pivot_time):
        ...
        task_ids.write({'last_export': pivot_time})
        task_ids.export_time_log_to_work()
    ...
    def delete_work_logs_on_host(self, ):
        log_by_task = defaultdict(lambda: self.env['work.time.log'])
        for log in self:
            log_by_task[log.task_id] |= log
        for task, logs in log_by_task.items():
            task.host_id.delete_time_logs(task, logs)

    def write(self, values):
        if 'task_id' in values and not self._context.get('bypass_auto_delete'):
            to_delete_logs = self.filtered(lambda r: r.task_id.id != values['task_id'] and r.export_state)
            _logger.info(to_delete_logs) 
            if to_delete_logs:
                to_delete_logs.delete_work_logs_on_host()
                self.env.cr.execute("""
        ...
        if 'export_state' not in values and not self._context.get("bypass_exporting_check"):
            processed_records = self.env['work.time.log']
            employee = self.env.user.employee_id
            if employee.auto_export_work_log:
                for log in self:
                    if log.task_id.host_id.auto_export_work_log:
                        try:
                            log.task_id.host_id.export_specific_log(log.task_id, log)
                            processed_records |= log
                        except Exception as e:
                            _logger.error(e)
            exported_values = {**values, **{'export_state': 1}}
            if processed_records:
                super(WorkTimeLog, processed_records.with_context(bypass_exporting_check=True)).write(exported_values)
            exported_logs = (self-processed_records).filtered(lambda r: r.export_state >= 1)
            if exported_logs:
                log_by_state = defaultdict(lambda: self.env['work.time.log'])
                ...
                for state, logs in log_by_state.items():
                    ...
                    super(WorkTimeLog, logs.with_context(bypass_exporting_check=True)).write(exported_values)
            to_update_records -= (processed_records | exported_logs)
        if to_update_records:
            res = super(WorkTimeLog, to_update_records).write(values)
    ...
    def force_export(self):
        ...
        for task in tasks.keys():
            task.host_id.export_specific_log(task, tasks[task])
        self.export_state = 1
        return self
    ...
    def unlink(self):
        try:
            if self.id_onhost:
                self.task_id.host_id.delete_time_logs(self.task_id, self)
        except:
            pass
        return super().unlink()
    ...
    def compare_with_external(self, keys={'start_date', 'duration', 'description'}):
        if self:
            ...
            # logs_by_ = defaultdict(lambda: self.env['work.time.log'])
            # for log in self:
            #     logs_by_[log.task_id._id] |= log
            # work_datas = []
            # for , logs in logs_by_.items():
            #     work_datas.append(.load_work_log_by_ids_raw(logs.mapped('id_onhost'), self.env.user))
            # raw_log_by_work_id = dict()
            # for log in work_datas:
            #     raw_log_by_work_id[log['id_onhost']] = log
            # res = defaultdict(dict)
            # for log in self:
            #     res[log.id] = None
            #     if raw_log_by_work_id(log.id_onhost):
            #         res[log.id] = {}
            #         for key in keys:
            #             res[log.id][key] = (log.key, raw_log_by_work_id[log.id_onhost].get(key))
            return res

    def action_export_work_logs(self):
        logs_by_user = defaultdict(lambda: self.env['work.time.log'])
        for log in self:
...
def write(self, values):
    return super(WorkTimeLogBase, self).write(values)

WorkTimeLogBase.write = write