...
class JiraTicket(http.Controller):
    ...
    def _get_ticket(self, ticket_ids):
        ...
        for ticket_id in ticket_ids:
            res.append({
                ...
                "last_start": ticket_id.last_start
            })
        return res

    @http.route(['/management/ticket/get/<int:ticket_id>'], type="json", methods=['POST'], auth='jwt')
    ...
    @http.route(['/management/ticket/get-my-all'], type="json", methods=['POST'], auth='jwt')
    ...
    @http.route(['/management/ticket/search/<string:keyword>'], type="json", methods=['POST'], auth='jwt')
    def search_ticket(self, keyword):
        ticket_ids = request.env['jira.ticket'].search(
            ['|', ('ticket_name', 'ilike', keyword), ('ticket_key', 'ilike', keyword)])
        data = self._get_ticket(ticket_ids)
        res = {
            "status": 200,
            "message": "Success",
    ...
    @http.route(['/management/ticket/work-log/add'], type="json", methods=['POST'], auth='jwt')
    ...
    @http.route(['/management/ticket/work-log/pause'], type="json", methods=['POST'], auth='jwt')
    def pause_ticket_work_log(self):
        ticket_id = self.__check_work_log_prerequisite()
        ticket_id.action_pause_work_log(request.params.get('payload', {}))
        return self._get_ticket(ticket_id)

    @http.route(['/management/ticket/work-log/done'], type="json", methods=['POST'], auth='jwt')
