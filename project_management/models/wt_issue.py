...
class WtProject(models.Model):
    ...
    issue_key = fields.Char(string='Issue Key', copy=False)
    issue_url = fields.Char(string='Task Issue', copy=False)
    epic_id = fields.Many2one("wt.issue", string="Epic")
    epic_ok = fields.Boolean(string="Epic?", related="issue_type_id.epic_ok", store=False)
    time_log_ids = fields.One2many('wt.time.log', 'issue_id', string='Log Times', copy=False)
    ...
    ac_ids = fields.One2many("wt.ac", "issue_id", string="Checklist", copy=False)
    ...
    status_id = fields.Many2one('wt.status', string='Status', copy=False)
    duration = fields.Integer('Duration', compute='_compute_duration', store=True, copy=False)
    progress_cluster_id = fields.Many2one('wt.work.log.cluster', string='Progress Cluster', copy=False)
    ...
    issue_sequence = fields.Integer('Issue Sequence', compute='_compute_issue_sequence', store=True, copy=False)
    ...
    priority_id = fields.Many2one("wt.priority", string="Priority", copy=False)
