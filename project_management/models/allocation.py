...
class WtAllocation(models.Model):
    ...
    finance_id = fields.Many2one(comodel_name="wt.finance", string="Finance Plan", required=True)
    project_id = fields.Many2one(comodel_name="wt.project", string="Project", required=True)
    ...
    allocation_group_id = fields.Many2one(comodel_name="wt.allocation.group", string="Allocation Group")

class WtAllocationGroup(models.Model):
    _name = "wt.allocation.group"
    ...
    allocation_ids = fields.One2many("wt.allocation", "allocation_group_id", string="Allocations")
    ...
    def _action_generate_allocations(self):
        ...
        return self.env['wt.allocation'].create(allocation_values)

    def action_generate_allocations(self):
        allocations = self.env['wt.allocation']
        for group in self:
